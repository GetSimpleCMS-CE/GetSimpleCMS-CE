...
while ($file = readdir($dir_handle)) {
	if ($file == "." || $file == ".." || $file == ".htaccess") {
		...
		if (in_array($ext, ['jpg', 'jpeg', 'gif', 'webp', 'png', 'svg'])) {
			$ss = @stat($path . $file);
			...
			<div class="main" style="border:none;">
				...
				<table class="highlight" id="imageTable">
					<tbody>
						...
						?>
								<tr class="All">
									<td colspan="5">
										<a href="imagebrowser.php?path=<?php echo htmlspecialchars($p, ENT_QUOTES, 'UTF-8'); ?>&amp;func=<?php echo htmlspecialchars($func, ENT_QUOTES, 'UTF-8'); ?>&amp;w=<?php echo $w; ?>&amp;h=<?php echo $h; ?>&amp;autoclose=1" title="<?php echo htmlspecialchars($upload['name'], ENT_QUOTES, 'UTF-8'); ?>"><svg xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;" width="32" height="32" viewBox="0 0 80 80"><rect width="80" height="80" fill="none"/><g fill="none"><path fill="#f2994a" fill-rule="evenodd" d="M16 17a4 4 0 0 0-4 4v38a4 4 0 0 0 4 4h48a4 4 0 0 0 4-4V29a4 4 0 0 0-4-4H35.4c-.367 0-.711-.177-.924-.475l-.34-.474l-.376-.526l-.377-.525l-3.099-4.329A4 4 0 0 0 27.032 17z" clip-rule="evenodd"/><rect width="56" height="38" x="12" y="25" fill="#f2c94c" rx="4"/></g></svg> <strong><?php echo htmlspecialchars($upload['name'], ENT_QUOTES, 'UTF-8'); ?></strong></a>
									</td>
								</tr>
						<?php
							}
								...
								<tr class="All images">
									<td style="vertical-align:middle;">
										<a href="javascript:void(0)" title="<?php echo htmlspecialchars(i18n_r('SELECT_FILE') . ': ' . $upload['name'], ENT_QUOTES, 'UTF-8'); ?>" onclick="<?php echo htmlspecialchars($onclick, ENT_QUOTES, 'UTF-8'); ?>">
											<img style="width:60px;height:60px;object-fit:cover;padding:3px;border:solid 1px #ccc" src="<?php echo htmlspecialchars($SITEURL . 'data/uploads/' . $subPath . $upload['name'], ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($upload['name'], ENT_QUOTES, 'UTF-8'); ?>" />
										</a>
									</td>
									<td style="vertical-align:middle;">
										...
										<p>
											<?php if ($upload['title'])       echo '<b>' . htmlspecialchars($upload['title'], ENT_QUOTES, 'UTF-8') . '</b><br/>'; ?>
											<?php if ($upload['tags'])        echo '<i>' . htmlspecialchars(implode(', ', (array) $upload['tags']), ENT_QUOTES, 'UTF-8') . '</i><br/>'; ?>
											<?php if ($upload['description']) echo preg_replace('/\r?\n/', '<br/>', htmlspecialchars($upload['description'], ENT_QUOTES, 'UTF-8')); ?>
										</p>
									</td>
				...
				<script>
					function submitLink(e) {
						var linker = document.querySelectorAll('.images img')[e].getAttribute('src');
