<?php

$i18n = [
	'MAITENANCETITLE' 				=> 'Maintenance Mode Options',
	'MAINTENANCE_ON' 				=> 'Maintenance Mode On?',
	'CONTENT_MAINTENANCE_MODE' 		=> 'Maintenance Mode Content:',
	'TURNONBOOTSTRAPGRID' 			=> 'Turn on Bootstrap-Grid 4.0?',
	'TURNONBOOTSTRAPGRIDONTHEME'	=> 'Add Bootstrap-Grid CSS to front-end theme?',
	'YES' 							=> 'Yes',
	'NO' 							=> 'No',
	'SAVEOPTION' 					=> 'Save Changes',
	'SUPPORT' 						=> 'If you want to support my work via PayPal :) Thanks! ',
	'UPLOADFILE' 					=> 'Upload file',
	'ANOTHERPAGE' 					=> '<strong>Changes Saved!</strong> Go to another page:)',
	'EDITCOMPONENTS'				=> 'Edit components',
	'MASSIVEADMINSETTINGSTITLE' 	=> 'MassiveAdmin Settings',
	'DROPFILES' 					=> 'Drop files here to upload',
	'BOOTSTRAPTITLE' 				=> 'Bootstrap-Grid Options',

	//options on file edit
	'COPYFILE' 						=> 'Copy file',
	'RENAMEFILE' 					=> 'Rename / Change Dir',
	'DOWNLOADFILE' 					=> 'Download',
	'INFOCOPY'						=> 'Your copied file is located in ',
	'INFOERROR' 					=> 'A file with this name already exists in this directory ',
	'FILENOW' 						=> 'Your file is located in ',

	//options on Create New User
	'CREATENEWUSER' 				=> 'Create New User',
	'USERNAMECREATE' 				=> 'User Name:',
	'PASSWORDCREATE' 				=> 'Password:',
	'EMAILCREATE' 					=> 'New User E-mail:',
	'LANGCREATE' 					=> 'User Language:',

	//options hidden section
	'HIDEMENUTITLE' 				=> 'User Manager & Sections',
	'SHOW' 							=> 'Show',
	'HIDE' 							=> 'Hide',
	'HIDSECTIONTITLE' 				=> 'What sections do you want to hide?',
	'HIDEPAGES' 					=> 'Hide "<strong>Pages</strong>" tab?',
	'HIDEFILES' 					=> 'Hide "<strong>Files</strong>" tab?',
	'HIDETHEMES' 					=> 'Hide "<strong>Themes</strong>" tab?',
	'HIDEBACKUP' 					=> 'Hide "<strong>Backup</strong>" tab?',
	'HIDEPLUGIN' 					=> 'Hide "<strong>Plugin</strong>" tab?',
	'HIDESUPPORT' 					=> 'Hide "<strong>Support</strong>" page?',
	'HIDESETTINGS' 					=> 'Hide "<strong>MassiveAdmin Settings</strong>" sections?',
	'HIDEGSSETTINGS'				=> 'Hide the "<strong>Settings</strong>" page?',
	'HIDEI18NGALLERY' 				=> 'Hide "<strong>i18n gallery</strong>" tab? (if exists)',
	'HIDEUSERS' 					=> 'Hide sections for the following users:',

	//user manager
	'USERMANAGER' 					=> 'Manage Users',

	//user help settings
	'USERHELPTITLE' 				=> 'User Help Settings',
	'TURNON' 						=> 'Turn On?',
	'HELP' 							=> 'Help',
	'USERHELP' 						=> 'User Help',

	//menu external
	'MENUEXTERNAL' 					=> 'Custom Menu Link Manager',
	'LINKNAME' 						=> 'Link Name:',
	'LINKURL' 						=> 'Link URL:',
	'LINKICON' 						=> 'Link Icon',
	'NEWWINDOW' 					=> 'Open link in new window?',
	'MOREICONS' 					=> '(Find more icons here.):',
	'ADDLINK' 						=> 'Add New Link',

	//own footer option
	'OWNFOOTERTITLE' 				=> 'White Label Options',
	'OWNFOOTERNAME' 				=> 'Custom Footer Name:',
	'OWNFOOTERICON' 				=> 'Custom Footer Icon (max 256px height | .jpg, .jpeg, .png, .gif)',
	'OWNLOGO' 						=> 'Custom Logo on Login Page?',
	'OWNFOOTERLINK' 				=> 'Custom Footer Link:',
	'OWNFOOTERHEADER' 				=> 'Custom Header CSS/Scripts:',
	'OWNFOOTERFOOTER' 				=> 'Custom Footer Scripts:',

	//owncolor
	'CHANGETITLECOLOR' 				=> 'Change CMS Colors',
	'MAINCOLOR' 					=> 'Main Color:',
	'BGCOLOR' 						=> 'Background Color:',
	'CHANGENAME' 					=> 'change the nickname to something other than the administrator nickname',

	//snippet
	'SNIPPET' 						=> 'Snippets',
	'ADDSNIPPET' 					=> 'Add Snippet',
	'EDITSNIPPET' 					=> 'Edit Snippets',
	'TITLESNIPPET' 					=> 'Title (without spaces)',
	'SUBMITSNIPPET' 				=> 'Save Snippets',

	//migrate
	'MIGRATETITLE' 					=> 'Site Migration & SSL',
	'MIGRATEDOMAIN' 				=> 'Domain Migration',
	'OLDDOMAIN' 					=> 'Old Website Address',
	'NEWDOMAIN' 					=> 'New Website Address',
	'REPLACEDOMAIN' 				=> 'Replace URL',
	'FORCESSL' 						=> 'Force SSL',
	'TURNONSSL' 					=> 'Force SSL in URL?',
	'SAVESSL' 						=> 'Save SSL settings',

	//plugin downloader
	'PLUGINDOWNLOADER' 				=> 'Download CE Plugins',
	'DOWNLOADERBASED' 				=> 'Based on the Get Simple CMS CE plugin repository.',
	'SEARCHPLUGIN' 					=> 'Search plugins...',
	'DOWNLOAD' 						=> 'Download',

	//login options
	'LOGINOPTIONS' 					=> 'Login Options',
	'TURNONSHOWPASSWORDTITLE' 		=> '"Show password" button',
	'SHOWPASSWORDQUESTION' 			=> 'Enable the "Show Password" button when logging in?',
	'REMOVEFORGETPASSWORDTITLE'		=> 'Remove "Forgot Password?"',
	'SAVESETTINGS' 					=> 'Save Settings',

	//GSCONFIG
	'GSCONFIGTITLE' 				=> 'Edit GSCONFIG settings',
	'GSCONFIGSAVE' 					=> 'Save the GSCONFIG settings',

	'UNISTALLER' 					=> 'Uninstall plugins',
	'UNISTALLQUESTION'				=> 'You want to uninstall',

	//mToper
	"TAB_PAGES"						=> 'Pages',
	"TAB_FILES"						=> 'Files',
	"TAB_THEME"						=> 'Theme',
	"TAB_BACKUPS"					=> 'Backups',
	"PLUGINS_NAV" 					=> 'Plugins',
	"TAB_SETTINGS"					=> 'Settings',
	"TAB_LOGOUT"					=> 'Logout',

	//makeFileInTheme
	"CREATENEWFILE"					=> 'Create New File',
	"FILECREATED"					=> 'File Created!',
	
	//frontend settings


	//new on 4.0

	//admin settings

	'FRONTENDTITLE' 				=> 'Front End Settings',
	'TURNONMTOPER' 					=> 'Turn on frontend bar?',
	'STYLEINFO' 					=> 'Style for FrontEnd sidebar',
	'ADMINTHEMESELECTOR' 			=> 'Admin Theme Selector',

	//manage users

	'EMAIL' 						=> 'e-mail:',
	'PASSWORD' 						=> 'Password:',
	'LANG' 							=> 'Language:',
	'CHANGEPLACEHOLDER' 			=> 'change only if you want to change',


	//new on 5.0

	'FILECREATED' 					=> 'File Created!',
	'WATCHEXAMPLE' 					=> 'View example structure',
	'HOWUSETHEMECONFIG' 			=> 'If you want to use settings for your template, just create <code style="color:blue;">settings.json</code> in the folder of the template you are using and apply the appropriate structure as in the example. Then apply the <b style="color:green;">mats("fieldname")</b> or <b style="color:green;">r_mats("fieldname")</b> for return function to the appropriate fields.',
	'HOWCREATETITLE' 				=> 'How to create your own settins?',
	'TUTORIALHOWCREATESETTINGS' 	=> 'Create your <code style="color:blue;">settings.json</code> in the root of your theme folder, with a structure based on this example with different types of options',
	'THEMECONFIGURATORNAME' 		=> 'Theme Configurator',

	'BACKUPCREATOR' 				=> 'Backup Creator',
	'CREATEBACKUP' 					=> 'Create Backup',
	'SETTINGS' 						=> 'Settings',
	'NOSETTINGSCREATED' 			=> 'No settings.json created.',
	'SHOWPASSWORD' 					=> 'Show Password'

];