...
function gs_setcookie($id, $value) {
	...
	$expire   = time() + ($cookie_time ?? 3600);
	$domain   = $cookie_domain   ?? '';
	$secure   = $cookie_secure   ?? false;
	$httponly = $cookie_httponly ?? true;
	return setcookie($id, $value, $expire, $cookie_path ?? '/', $domain, $secure, $httponly);
}
/**
 * Unset a gs cookie
 * @since  3.3.5
...
function gs_unsetcookie($id) {
	...
	$secure   = $cookie_secure   ?? false;
	$httponly = $cookie_httponly ?? true;
	return setcookie($id, '', 1, $cookie_path ?? '/', $domain, $secure, $httponly);
}
/**
 * Create Cookie
 *
...
function create_cookie() {
	...
	if (empty($USR) || empty($SALT)) return false; 
...
function cookie_check() {
	global $SALT, $cookie_name;

	// Get username from cookie - don't rely on global $USR
	$usr = isset($_COOKIE['GS_ADMIN_USERNAME'])
		? strtolower(preg_replace('/[^a-zA-Z0-9_\-]/', '', $_COOKIE['GS_ADMIN_USERNAME']))
		: '';

	if (empty($usr) || empty($SALT)) return false;

	$saltCOOKIE = sha1($cookie_name . $SALT);
	if (!isset($_COOKIE[$saltCOOKIE])) return false;

	return $_COOKIE[$saltCOOKIE] === sha1($usr . $SALT);
}
